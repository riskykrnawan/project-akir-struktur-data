...
void addUser(Node_1 **head, Node_1 **tail, int &accountCount)
{
	...
	{ // kalau linked list sudah terisi
		newNode->next = NULL;
		(*tail)->next = newNode;
		*tail = newNode;
	}
	...
	while (TOP != NULL)
	{
		...
		if (TOP->data.ID == selectedID)
		{
			do
			{
				...
				{
					cout << "\n\nMasukkan Tanggal Rilis Baru :  ";
					fflush(stdin);
					getline(cin, TOP->data.tanggalRilis);
					repeat = false;
					cout << "\n\nBerhasil!!!\n\n\n\n"
						 << endl;
				}
...
void checkout(Node_1 *head, Node_2 *TOP, Node_3 **FRONT, string userName)
{
	...
	while (repeat)
	{
		...
		cout << "Apakah Anda Yakin Membeli Semua Game Ini? (Y/N) : ";
		pil = getch();
